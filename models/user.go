...
//单个信息
func QueryUser(username string) (userInfo *User, err error) {
	userInfo = new(User) //这里要新建对象
	if err = dao.DB.Where("username = ?", username).First(&userInfo).Error; err != nil {
		fmt.Println("Error of QueryUser:", err)
		return nil, err
	}
	return
}
